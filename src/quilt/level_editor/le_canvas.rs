    ...
    pub fn show_editor_ui(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            egui::ComboBox::from_label("Selected file")
                .selected_text(&self.archive_contents[self.selected_file_index].0)
                .show_ui(ui, |ui| {
                    let mut index = self.selected_file_index;
                    for i in 0..self.archive_contents.len() {
                        ui.selectable_value(&mut index, i, &self.archive_contents[i].0);
                    }
                    ...
                    if self.selected_file_index != index {
                        ...
                        self.selected_mapbin_index = if index.is_multiple_of(2)
            ...
            ui.checkbox(&mut self.display_none, "Display 'NONE'?")
                .on_hover_text(
                    "Indicates whether or not to display entities with a name of 'NONE'.",
                );
            ...
            if ui
                ...
                .clicked()
            {
                self.current_mapdata
                    .gimmicks
                    .retain(|gimmick| gimmick.name != "NONE");
                self.current_mapdata
                    .paths
                    .retain(|path| path.name != "NONE");
                self.current_mapdata
                    .zones
                ...
                && let Some(mut pointer_pos) = response.hover_pos()
            {
                ...
                match object_type {
                    ...
                    ObjectType::CommonGimmick(hex) => {
                        ...
                        if !self
                            .current_mapdata
                            .common_gimmick_names
                            .hex_names
                            .iter()
                            .any(|g| g.as_str() == hex_str)
                        {
                            self.current_mapdata
                                .common_gimmick_names
                                .hex_names
